...
public class StarlightEngine {
    ...
    // index = x + (z * 5)
    protected final OChunk[] chunkCache = new OChunk[5 * 5];
    ...
    protected final OWorld world;

    protected StarlightEngine(final boolean skylightPropagator, final OWorld world) {
    ...
    protected final void setupCaches(final OWorld chunkProvider, final int centerX, final int centerY, final int centerZ,
        ...
        for (int dz = -radius; dz <= radius; ++dz) {
            for (int dx = -radius; dx <= radius; ++dx) {
                ...
                final OChunk chunk = !chunkProvider.f(cx, cz) ? null : chunkProvider.c(cx, cz);
    ...
    protected final OChunk getChunkInCache(final int chunkX, final int chunkZ) {
        return this.chunkCache[chunkX + 5*chunkZ + this.chunkIndexOffset];
    }

    protected final void setChunkInCache(final int chunkX, final int chunkZ, final OChunk chunk) {
        this.chunkCache[chunkX + 5*chunkZ + this.chunkIndexOffset] = chunk;
    }

    protected final ONibbleArray getNibbleFromCache(final int chunkX, final int chunkZ) {
        final OChunk chunk = this.getChunkInCache(chunkX, chunkZ);
        return chunk == null ? null : (this.skylightPropagator ? chunk.f : chunk.g);
    }

    protected final int getBlockState(final int worldX, final int worldY, final int worldZ) {
        final OChunk chunk = this.getChunkInCache(worldX >> 4, worldZ >> 4);

        if (chunk == null || worldY < 0 || worldY > 127) {
            return AIR_BLOCK_STATE;
        }

        return chunk.a(worldX & 15, worldY & 127, worldZ & 15);
    }


    protected final int getLightLevel(final int worldX, final int worldY, final int worldZ) {
        final ONibbleArray nibble = this.getNibbleFromCache(worldX >> 4, worldZ >> 4);

        if (nibble != null && worldY >= 0 && worldY <= 127) {
            return nibble.a(worldX & 15, worldY & 127, worldZ & 15);
        }

        if (this.skylightPropagator) {
            return worldY > 127 ? 15 : (nibble == null ? 15 : Math.max(0, nibble.a(worldX & 15, 0, worldZ & 15) - worldY)); // best approximation
        } else {
            return 0;
        }
    }
    ...
    protected final void setLightLevel(final int worldX, final int worldY, final int worldZ, final int level) {
        ...
        if (nibble != null && worldY >= 0 && worldY <= 127) {
            nibble.a(worldX & 15, worldY & 127, worldZ & 15, level);
        }
    }
    ...
    //int f = OBlock.s[4]; // emitted light
    // int f = OBlock.q[4]; // subtracted light

    protected final void performLightIncrease(final OWorld world) {
        long[] queue = this.increaseQueue;
        ...
        while (queueReadIndex < queueLength) {
            ...
            for (final AxisDirection propagate : checkDirections) {
                ...
                final ONibbleArray currentNibble = this.getNibbleFromCache(offX >> 4, offZ >> 4);
                final int currentLevel;
                if (currentNibble == null || (currentLevel = currentNibble.a(offX & 15, offY & 127, offZ & 15)) >= (propagatedLightLevel - 1)) {
                    continue; // already at the level we want or unloaded
                }

                final int blockState = this.getBlockState(offX, offY, offZ);
                final int opacityCached = OBlock.q[blockState];

                final int targetLevel = propagatedLightLevel - Math.max(1, opacityCached);
                if (targetLevel > currentLevel) {
                    currentNibble.a(offX & 15, offY & 127, offZ & 15, targetLevel);
    ...
    protected final void performLightDecrease(final OWorld world) {
        long[] queue = this.decreaseQueue;
        ...
        while (queueReadIndex < queueLength) {
            ...
            for (final AxisDirection propagate : checkDirections) {
                ...
                if (currentNibble == null || (lightLevel = currentNibble.a(offX & 15, offY & 127, offZ & 15)) == 0) {
                ...
                final int emittedLight = OBlock.s[blockState] & emittedMask;
                if (emittedLight != 0) {
                    // re-propagate source
                ...
                currentNibble.a(offX & 15, offY & 127, offZ & 15, 0);
                this.postLightUpdate(offX, offY, offZ);
    ...
    protected final void checkBlockEmittance(final int worldX, final int worldY, final int worldZ) {
        ...
        try {
            ...
            final int emittedLevel = OBlock.s[blockState] & emittedMask;
    ...
    protected final void checkSkyEmittance(final int worldX, final int worldY, final int worldZ) {
        this.setupCaches(this.world, worldX, worldY, worldZ, true);
        try {
            ...
            if (this.getLightLevel(worldX, maxPropagationY, worldZ) == 15) {
                ...
                for (int currY = maxPropagationY; currY >= 0; --currY) {
                    if (nibble.a(worldX & 15, currY & 127, worldZ & 15) != 15) {
                        break;
                    }
                    ...
                    nibble.a(worldX & 15, currY & 127, worldZ & 15, 0);
                }
            }
    ...
    protected final int tryPropagateSkylight(final OWorld world, final int worldX, int startY, final int worldZ) {
        final int encodeOffset = this.coordinateOffset;
        ...
        for (;startY >= 0; --startY) {
            ...
            final int opacityIfCached = OBlock.q[current];
    ...
    public final void initBlockLight(final int chunkX, final int chunkZ) {
        this.setupCaches(this.world, (chunkX << 4) | 7, 64, (chunkZ << 4) | 7, true);
        try {
            if (this.world.m.e) { // isHell
                final OChunk chunk = this.getChunkInCache(chunkX, chunkZ);
                final ONibbleArray nibble = this.getNibbleFromCache(chunkX, chunkZ);
                final int encodeOffset = this.coordinateOffset;

                for (int y = 0; y <= 127; ++y) {
                    for (int z = 0; z <= 15; ++z) {
                        for (int x = 0; x <= 15; ++x) {
                            final int blockState = chunk.a(x, y, z);
                            if (blockState == AIR_BLOCK_STATE) {
                                continue;
                            }
                            final int emittedLight = OBlock.s[blockState];

                            if (emittedLight != 0) {
                                nibble.a(x, y, z, emittedLight);
                                this.appendToIncreaseQueue(
    ...
    protected final void propagateNeighbourLevels(final int chunkX, final int chunkZ) {
        final ONibbleArray currentNibble = this.getNibbleFromCache(chunkX, chunkZ);
        for (int currSectionY = (127 >> 4); currSectionY >= 0; --currSectionY) {
            for (final AxisDirection direction : ONLY_HORIZONTAL_DIRECTIONS) {
                ...
                final ONibbleArray neighbourNibble = this.getNibbleFromCache(chunkX + neighbourOffX,
                ...
                for (int currY = currSectionY << 4, maxY = currY | 15; currY <= maxY; ++currY) {
                    for (int i = 0, currX = startX, currZ = startZ; i < 16; ++i, currX += incX, currZ += incZ) {
                        final int level = neighbourNibble.a(currX & 15, currY & 127, currZ & 15);
